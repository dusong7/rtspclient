...
char *b64_encode(const char *buf, int len) {
    int elen;
    char *out;
...
char *b64_decode_ex(const char *buf, int n) {
    int elen;
    char *out;

    elen = apr_base64_decode_len(buf, n);
    out = (char *) malloc(sizeof(char) * (elen + 1));

    apr_base64_decode(out, buf, n);

    return out;
}

char *b64_decode(const char *buf) {
	return b64_decode_ex(buf, strlen(buf));
}
