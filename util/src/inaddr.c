...
int inet_aton (const char *name, struct in_addr *addr){
	unsigned long ad = inet_addr(name);
	if(ad == INADDR_NONE)
		return 0;
	addr->s_addr = (in_addr_t)ad;
	return 1;
}
#endif
...
{
#ifndef _WIN32
    if(src[0] == '@' || strncmp(src, "unix:", 5) == 0){
    	// this is a local socket
    	struct sockaddr_un* addr = (struct sockaddr_un*)dst;
    	addr->sun_family = AF_UNIX;
    	if(src[0] != '@')
    		src += 5;
    	memset(addr->sun_path, 0, sizeof(addr->sun_path));
    	strncpy(addr->sun_path, src, sizeof(addr->sun_path));
    	if(addr->sun_path[0] == '@')
    		addr->sun_path[0] = '\0';
    	return 1;
    }
...
const char *j_inet_ntop(struct sockaddr_storage *src, char *dst, size_t size)
{
	...
	if(src->ss_family == AF_UNIX){
		struct sockaddr_un *addr = (struct sockaddr_un *)src;
		if(size < sizeof(addr->sun_path) + 6)
			return NULL;
		memcpy(dst, "unix:", 5);
		memcpy(dst + 5, addr->sun_path, sizeof(addr->sun_path));
		dst[5 + sizeof(addr->sun_path)] = '\0';
		if(dst[5] == '\0')
			dst[5] = '@';
		return dst;
	}
...
#include <netdb.h>
#endif
char* j_inet_getaddr(const char* hostname){
	struct addrinfo hint={0}, *res = NULL, *next;
	int ret;
	int try = 5;
	...
	}while(ret == EAI_AGAIN && try-- > 0);

	if(0 == ret){
		for(next = res; next; next = next->ai_next){
			char ipaddr[20];
			j_inet_ntop((struct sockaddr_storage*)next->ai_addr, ipaddr, sizeof(ipaddr));
			freeaddrinfo(res);
