...
JABBERD2_API nad_t nad_get_subnad(nad_t nad, int elem);
...
/* same as nad_find_elem_path,but i wonder if it's a bug that nad_find_elem_path will only n return child elem ,never return child's child ,if i write "name1/name2/name3",i want get name3,bug nad_find_elem_path will return name1. so i create this guy.*/
JABBERD2_API int nad_find_path_elem(nad_t nad, int elem, int ns, const char *name);
...
JABBERD2_API int nad_insert_elem(nad_t nad, int parent, int ns, const char *name, const char *cdata);
...
JABBERD2_API void nad_print_doc(nad_t nad, int elem, char **xml, int *len);
...
JABBERD2_API int nad_check_deserializable(const char*, int);

/** create a nad from raw xml */
JABBERD2_API nad_t nad_parse(const char *buf, int len);

JABBERD2_API void nad_set_cdata(nad_t nad, int elem, const char *cdata, int len);
