...
int jqueue_locate(jqueue_t q, void *data){
    _jqueue_node_t scan;
    int pos = 0;
    if(q->back == NULL && q->front == NULL){
    	return -1;
    ...
    /* find the node with data equal to us */
    for(scan = q->back; scan != NULL && scan->data != data; scan = scan->next) ++pos;
...
void jqueue_remove(jqueue_t q, void *data){
    ...
    /* find the node with data equal to us */
    for(scan = q->back; scan != NULL && scan->data != data; scan = scan->next);
